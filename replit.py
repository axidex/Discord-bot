...
bot_channel_name = os.environ['bot_channel_name']
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for admin in admins:
      ...
      return await ctx.send(
          f'<:gachiFU:998171613247848448> Fuck you!!!!!!, {ctx.message.author.mention}! You not my master FUCKING SLAVE! <:gachiFU:998171613247848448>'
      )
    ...
    await ctx.send(
        f'Ohh my master {ctx.message.author.mention}, this dirty {user} was added to my slaves note.'
    )
...
async def unban(ctx, ban_mode, user):
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    await ctx.send(
        f'Ohh my lord {ctx.message.author.mention}, this {user} is now free.')
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      if ban == f"{ctx.message.author.id}":
        return await ctx.send(
            f'Fuck you, {ctx.message.author.mention}! <:gachiFU:998171613247848448>'
        )
    else:
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      ...
      response = requests.get(
          'https://some-random-api.ml/img/fox')  # Get-запрос
      json_data = json.loads(response.text)  # Извлекаем JSON

      embed = discord.Embed(color=0xff9900,
                            title='Random joke')  # Создание Embed'a
...
def playNext(ctx):
  ...
  if len(song_queue) > 0:
    ...
    voice_client.play(discord.FFmpegPCMAudio(executable=ffmpeg_path,
                                             source=URL,
                                             **FFMPEG_OPTIONS),
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      ...
      if arg is None and not song_queue:
        return await ctx.send(
            f"Hey {ctx.message.author.mention}, you must include something to play or i will cum inside your ass!"
        )
      else:
        song_queue.append(arg)
        await ctx.send(
            f"``{arg}`` Thank you sir {ctx.message.author.mention}! I added your song to queue. <:gachiVan:998171539746861108>"
        )
    ...
    for ban in bans:
      ...
      for ban in skipBans:
        ...
        if voice:
          ...
          await ctx.send(
              f"YeASS sir {user}, song is paused. <:gachiSleeper:998171510990708817>"
          )
    ...
    for ban in bans:
      ...
      for ban in skipBans:
        ...
        if voice:
          ...
          user = ctx.message.author.mention
          await ctx.send(
              f"Oh my master {user}, song resumed! <:gachiVan:998171539746861108>"
          )
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      ...
      try:
        ...
        user = ctx.message.author.mention
        await ctx.send(
            f'``{url}`` Thank you sir {user}! I added your song to queue. <:gachiVan:998171539746861108>'
        )
      except:
    ...
    for ban in bans:
      ...
      for ban in skipBans:
        ...
        try:
          ...
          if len(song_queue) < 1:
            await ctx.send(
                "Your queue is empty now! Now, I'm ready to cum! <:gachiVan:998171539746861108>"
            )
          else:
            await ctx.send(
                f'Yeah, i did it! Now, swallow my cum <:gachiBASS:998171333127057428>: {song_queue}'
            )
        except:
          await ctx.send(
              "<:gachiFU:998171613247848448> Fuck you!!! My laptop says this shit: List index out of range - the queue starts at 0! <:gachiFU:998171613247848448>"
          )
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      ...
      for ban in skipBans:
        ...
        user = ctx.message.author.mention
        await ctx.send(
            f"Ohhh, thanks for free fisting {user} <:gachiBASS:998171333127057428>! Now, my ass is clean."
        )
  ...
  if ctx.channel.name == bot_channel_name:
    ...
    for ban in bans:
      ...
      for ban in skipBans:
        ...
        await ctx.send(
            f"Bye bye, my master! <:gachiSleeper:998171510990708817>")
...
class Google(commands.Cog):
  ...
  def parse_google_card(self, node):
    ...
    if time_in is not None:
      try:
        time_place = ''.join(
            time_in.find("span[@class='_HOb _Qeb']").itertext()).strip()
        the_time = ''.join(
            time_in.find("div[@class='_rkc _Peb']").itertext()).strip()
        the_date = ''.join(
            time_in.find("div[@class='_HOb _Qeb']").itertext()).strip()
      except:
        return None
      else:
        e.title = time_place
  ...
  async def g(self, ctx, *, query):
    ...
    try:
      ...
      e.set_author(
          ...
          icon_url=
          "https://cdn.discordapp.com/attachments/278603491520544768/300645219626647564/Google-logo-2015-G-icon.png"
      )
      if next_two:
  ...
  async def i(self, ctx, *, query):
    ...
    async with aiohttp.ClientSession() as cs:
      async with cs.get("https://www.googleapis.com/customsearch/v1?q=" +
                        query.replace(' ', '+') + "&start=" + '1' + "&key=" +
                        self.bot.google_api_key + "&cx=" +
                        self.bot.custom_search_engine +
                        "&searchType=image") as resp:
        if resp.status != 200:
...
asyncio.run(bot.add_cog(Google(bot)))
#t2 = threading.Thread(target=bot.add_cog, args=(Google(bot)))
